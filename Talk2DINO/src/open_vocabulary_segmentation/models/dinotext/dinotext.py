...
class DINOText(nn.Module):
    ...
    def __init__(
        ...
        if 'dinov2' in model_name:
            ...
            # 改为用户文件夹下的.cache路径
            path = os.path.expanduser('~/.cache/torch/hub/facebookresearch_dinov2_main')
            self.model = torch.hub.load(path, model_name, source = 'local')
        ...
        if 'bert' in self.clip_model_name:
            ...
            path = os.path.join(path, clip_model_name + '.pt')
            self.clip_model, _ = clip.load(path, device=device)
        ...
        project_path = 'Talk2DINO'
        config_path = os.path.join(project_path, "configs")
        with open(os.path.join(config_path, f"{proj_class}.yaml"), 'r') as config_file:
        ...
        if pre_trained:
            ...
            self.proj.load_state_dict(
                torch.load(os.path.join(weight_path, f"{proj_name}.pth"), 'cpu'))
        self.proj.to(device)
    ...
    def encode_text_similarity(self, texts):
    ...
    def encode_image_similarity(self, images):

        img_preprocessed = self.image_transforms(images).to(next(self.parameters()).device)
