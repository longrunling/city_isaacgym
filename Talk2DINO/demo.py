...
parser.add_argument(
    "--config", type=str, 
    default="Talk2DINO/src/open_vocabulary_segmentation/configs/cityscapes/dinotext_cityscapes_vitl_mlp_infonce.yml"
)
parser.add_argument("--output", type=str, default="Talk2DINO/TEST.png")
parser.add_argument("--input", type=str, default="rgb_image_cv2.png")
parser.add_argument("--with_background", action="store_true", default=True)
parser.add_argument("--textual_categories", type=str, default="house")
...
with torch.no_grad():
    ...
    mask, _ = model.generate_masks(
        img, img_metas=None, text_emb=text_emb, classnames=text, apply_pamr=True)
    if with_background:
