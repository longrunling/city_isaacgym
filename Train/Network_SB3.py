...
class Height_Map_Encoder(BaseFeaturesExtractor):
    def __init__(
        self,
        observation_space: gym.spaces.Space,
        feature_dim,
        ...
        super(Height_Map_Encoder, self).__init__(observation_space, feature_dim)
        ...
        dummy_input = torch.zeros(self.state_input_shape['occ_map'])
        dummy_input.unsqueeze_(0).unsqueeze_(1)
        # 增加 batch 维度和 channel 维度
        with torch.no_grad():
            conv_out = self.naive_encoder_grid(dummy_input)
            output_layer_size = conv_out.reshape(dummy_input.shape[0], -1)
            output_layer_size = output_layer_size.shape[1]

        self.output_layer_grid = nn.Sequential(
            nn.Linear(in_features=output_layer_size, out_features=256, bias=True),   # feature size after 2D CNN, grid_size = 20
            nn.ReLU(inplace=True),
        )

        action_size = self.state_input_shape['state'][1]
        dummy_input = torch.zeros(state_input_shape['state']).view(
            1, -1, action_size)
        ...
        self.naive_encoder_action = nn.Sequential(
            nn.Linear(
                in_features=in_feature_size, out_features=256, bias=True),   
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        # action
        state_size = self.state_input_shape['state'][0] * \
            self.state_input_shape['state'][1]
        action_size = self.state_input_shape['state'][1]
        action_input = observations[:, :state_size]  
        # [num_env, buffer_size*action_size]
        action_input = action_input.view(num_env, -1, action_size)
        action_input = self.positional_encoding(action_input).view(num_env, -1)
        feature_action = self.naive_encoder_action(action_input)    
        # [num_env, 256]

        # grid
        grid_size = self.state_input_shape['occ_map'][0] * \
            self.state_input_shape['occ_map'][1]
        grid_input = observations[
            :, state_size : state_size + grid_size]     
        # [num_env, 1*X*Y*Z]
        grid_input = grid_input.reshape(
            num_env, 1, 
            self.state_input_shape['occ_map'][0], 
            self.state_input_shape['occ_map'][1])
